...
public class OptionWall : OptionManager
{
    ...
    {
        CP.Constrain(true, false);
        if (translation || rotation)
            ChessPiece.Manipulated = CP;
        else ChessPiece.Manipulated = null;
    }

    public void ChangeSize(Vector2 rotateIncrement, bool left, bool right, bool up, bool down)
    {
        Vector2 DeltaTargetPos;
        Vector2 increment = MyMathf.rotate(rotateIncrement, -CP.transform.rotation.z * Mathf.Rad2Deg);
        increment = rotateIncrement;
        ...
        dir = MyMathf.rotate(dir, CP.transform.eulerAngles.z);
        ...
        if (!IncreaseSize || distance > 0)
        {
            ...
            DeltaTargetPos = MyMathf.rotate(DeltaTargetPos, CP.transform.rotation.eulerAngles.z);
        
            CP.TeleportTo(InitialPos + DeltaTargetPos);
        }
